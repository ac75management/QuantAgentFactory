...
## NEXT ACTION
Decidir si se invoca `engine` sobre `docs/specs/xauusd-d1-mean-reversion-streak-extension.md` ahora (Gate 0 dará `APTO_CON_RESERVAS` por slippage sin confirmar — spec ya lo anticipa y no lo trata como bloqueo duro) o si Alexander prefiere confirmar slippage antes. No invocar `validator` todavía (ningún OOS abierto). Revisar también la hipótesis #002 (`nas100-d1-turn-of-month-flow`) que apareció sola, generada desde la sesión de VS Code.
