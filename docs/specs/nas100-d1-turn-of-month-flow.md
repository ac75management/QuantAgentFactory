...
- Precio de referencia: **mid** — asunción interina, igual que en `docs/archive/specs/dryrun_bh_sp500.md` y `docs/specs/xauusd-d1-mean-reversion-streak-extension.md`, porque `data/clean/` no distingue bid/ask todavía. `engine` debe declarar el precio real (bid/ask/mid) en `reports/nas100-d1-turn-of-month-flow/data_quality.md` al correr Gate 0. Si Gate 0 declara algo distinto de mid, las fórmulas de fill y de costo de spread de esta sección deben ajustarse antes del backtest.
...
**No se ha corrido todavía comprar-y-mantener neto para NAS100** (a diferencia de SP500, XAUUSD y EURUSD, ya corridos en `reports/dryrun_costs_comparativa.md` — ver `PROJECT_STATE.md`). No se asume el resultado. Antes de que `validator` pueda aplicar esta puerta, se necesita un dry-run análogo (`dryrun_bh.py --alias NAS100`, script retirado; tag git `archive/pre-factory-v2`; mismo patrón que los 3 ya corridos) sobre `data/clean/NAS100/D1/OOS.parquet` específicamente (CLAUDE.md regla 19 exige la misma ventana OOS, no la ventana histórica completa).
