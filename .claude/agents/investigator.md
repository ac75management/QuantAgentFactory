...
2. Una lista de GitHub, un blog, Quantpedia o un informe/respuesta generado por IA (Gemini, ChatGPT u otra herramienta) son fuentes secundarias o terciarias, nunca evidencia primaria. Sigue cada afirmación relevante hasta el paper, libro, documento o código original y verifica autoría, fecha/versión, universo, periodo, reglas y datos en esa fuente. Usa el informe de IA solo como pista de búsqueda; no copies sus reglas ni métricas como verificadas. Si no existe una fuente primaria accesible para las afirmaciones esenciales, deja la entrada bloqueada; no promociones un resumen.
...
6. Una tarea `evidence_refresh` no crea otra hipótesis ni reemplaza evidencia. Compara el snapshot nuevo con la fuente ya revisada y, al terminar, ejecuta `.venv/Scripts/python.exe -m qaf.cli catalog-ack-refresh <candidate_id> <metadata_sha256>`. La huella reconocida queda en el candidato versionado para que perder SQLite no reabra trabajo ya atendido.
